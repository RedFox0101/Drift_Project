...
public class WheelSkid : MonoBehaviour {
	...
	[SerializeField] private float SKID_FX_SPEED = 0.5f; // Min side slip speed in m/s to start showing a skid
	...
	public event Action<float> OnSkidIntensity;
	public event Action<float> OnSkidTotal;

    // #### UNITY INTERNAL METHODS ####

    public void SetController(Skidmarks skidmarks)
    {
	...
	{
		if (skidmarksController == null) return;
		...
		if (wheelCollider.GetGroundHit(out wheelHitInfo))
		{
			...
			if (skidTotal >= SKID_FX_SPEED) {
				...
				if(SkidSound != null) SkidSound.volume = intensity / SkidSoundMultiplyer;
				
				OnSkidIntensity?.Invoke(intensity);
				OnSkidTotal?.Invoke(skidTotal / MAX_SKID_INTENSITY);
			}
			else {
				lastSkid = -1;
			}
